...
class State:
    ...
    def data_handler(self, ctx, data):
        ...
        # tracks total samples received by pi from sensor
        self.samples += 1
...
def streamData():
    ...
    while True: 
        userInput = input("Enter mode (q - end): ")
    
        modes = ['LGW','SA','SD','RA','RD']
    
        if userInput in modes:
                s.mode = modes.index(userInput) + 1
    ...
    for idx, s in enumerate(states):
        ...
        with open(filename, 'w') as f:
            f.write('epoch, elapseds, xacc, yacc, zacc, xgyro, ygyro, zgyro, mode\n')
            for row in s.sensor_data:
                f.write('%d,%.3f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%d\n' % (row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]))
